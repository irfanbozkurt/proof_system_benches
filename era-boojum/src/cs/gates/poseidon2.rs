    ...
    [(); PAR::NUM_PARTIAL_ROUNDS]:,
{
    ...
    pub fn compute_round_function<CS: ConstraintSystem<F>>(
        ...
        println!();
        println!(
            "total_num_variables = {}, reserved_for_in_out = {}",
            total_num_variables, reserved_for_in_out
        );
        println!();
...
mod test {
    ...
    use algebraic_props::round_function::AbsorptionModeOverwrite;
    use algebraic_props::sponge::GoldilocksPoseidonSponge;
    use cs_builder_verifier::CsVerifierBuilder;
    use field::goldilocks::{GoldilocksExt2, GoldilocksField};
    use implementations::pow::NoPow;
    use implementations::prover::ProofConfig;
    use implementations::transcript::GoldilocksPoisedonTranscript;
    ...
    #[test]
    fn test_simple_poseidon2_bench() {
        let geometry = CSGeometry {
            num_columns_under_copy_permutation: 80,
            num_witness_columns: 120,
        ...
        let builder = new_builder::<_, F>(
            CsReferenceImplementationBuilder::<F, F, DevCSConfig>::new(geometry, 128),
        );

        let builder = configure(builder);

        let mut owned_cs = builder.build(CircuitResolverOpts::new(1 << 24));
        ...
        for _ in 0..100 {
            let a = if let Some(previous) = previous.take() {
                ...
                cs.alloc_single_variable_from_witness(GoldilocksField::from_u64_with_reduction(1))
            };
            let b =
                cs.alloc_single_variable_from_witness(GoldilocksField::from_u64_with_reduction(2));
            let c =
                cs.alloc_single_variable_from_witness(GoldilocksField::from_u64_with_reduction(3));
            ...
            let e = ZeroCheckGate::check_if_zero(cs, d);

            previous = Some(e);
        }

        // make few constants
        cs.allocate_constant(GoldilocksField::from_u64_with_reduction(3));
        cs.allocate_constant(GoldilocksField::from_u64_with_reduction(4));
        ...
        let mut out_circuit_state = [F::ZERO; 12];
        for (idx, dst) in inputs.iter_mut().enumerate() {
            ...
            out_circuit_state[idx] = value;
            *dst = var;
        }

        let ignore_cells_state = [cs.allocate_constant(F::ZERO); 4];
        let round_function_result =
            Poseidon2Gate::compute_round_function(cs, inputs, ignore_cells_state);

        crate::implementations::poseidon2::poseidon2_permutation(&mut out_circuit_state);
        log!("Out of circuit result = {:?}", out_circuit_state);
        ...
        assert_eq!(circuit_result, out_circuit_state);
