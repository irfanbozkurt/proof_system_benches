...
func newSha256(api frontend.API) Sha256 {
	return Sha256{
		api:    api,
		uapi8:  NewUint8API(api),
		uapi32: NewUint32API(api),
	}
}
...
func blockGeneric(dig *Digest, chunkBytesToIgnore frontend.Variable, data ...Xuint8) {
	...
	for len(data) >= chunk {
		var w []Xuint32
		for i := 0; i < 16; i++ {
			chunk32 := []Xuint8{data[i*4], data[i*4+1], data[i*4+2], data[i*4+3]}
			w = append(w, uapi8.DecodeToXuint32BigEndian(chunk32))
		}
		w = append(w, make([]Xuint32, 48)...)
		for i := 16; i < 64; i++ {
			w[i] = ConstUint32(0)
		}
		...
		// note: size of data is guaranteed to be a multiple of 64 bytes
		shouldIgnoreChunk := gnark.IsZero(gnark.Sub(chunkBytesToIgnore, len(data)))
		/*
		   ...
		   h7 := h7 + h
		*/
		h0 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h0), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(a))), 33)
		h1 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h1), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(b))), 33)
		h2 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h2), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(c))), 33)
		h3 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h3), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(d))), 33)
		h4 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h4), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(e))), 33)
		h5 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h5), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(f))), 33)
		h6 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h6), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(g))), 33)
		h7 = sha.trimBitsToXuint32(gnark.Add(uapi32.FromUint32(h7), gnark.Select(shouldIgnoreChunk, 0, uapi32.FromUint32(h))), 33)

		chunkBytesToIgnore = gnark.Select(shouldIgnoreChunk, gnark.Sub(chunkBytesToIgnore, 64), chunkBytesToIgnore)
		data = data[chunk:]
	}
...
func (h *Sha256) rightRotate(n Xuint32, shift int) Xuint32 {
	return h.uapi32.Rrot(n, shift)
}

func (h *Sha256) computeMaj(a, b, c Xuint32, tempMaj1, tempMaj2 *Xuint32, useLazy bool) Xuint32 {
	var res Xuint32
	for i := range res {
		if useLazy {
...
func (h *Sha256) computeCh(e, f, g Xuint32) Xuint32 {
	var res Xuint32
	for i := range res {
...
func (h *Sha256) rightShift(n Xuint32, shift int) Xuint32 {
...
func (h *Sha256) trimBitsToXuint32(a frontend.Variable, size int) Xuint32 {
	requiredSize := 32
