...
//     fn fibo_snark(num_rows: usize) -> (CircuitBuilder<F, D>, PartialWitness<F>) {
...
//         for _ in 0..100 {
//             prev_target = initial_a;
//             cur_target = initial_b;
...
//         // Pure SNARK
//         let (builder, pw) = fibo_snark(num_rows);
...
//         let start_time = Instant::now();
//         data.verify(proof);
//         println!("fibo SNARK verifying time: {:?}", start_time.elapsed());
...
//         // build inner proof, and finally a proof to be verified in-circuit - parallelizable
//         let start_time = Instant::now();
//         let trace = stark.generate_trace(public_inputs[0], public_inputs[1]);
...
//             &mut TimingTree::default(),
//         )?;
//         // Create a proof to be verified in-circuit.
