...
pub struct SemaphoreTargets {
    merkle_root: HashOutTarget, // 256bit hash output
    topic: [Target; 4],
...
impl AccessSet {
    pub fn circuit(&self, builder: &mut CircuitBuilder<F, 2>) -> SemaphoreTargets {
        // Register public inputs
        // Virtual hash means we don't know at which row they will be. They
        // will be "wired" automatically later.
        ...
        // Verify Merkle proof for public key
    ...
    pub fn fill_targets(
        ...
        partial_witness.set_hash_target(merkle_root, self.0.cap.0[0]);
        partial_witness.set_target_arr(&private_key_target, &private_key);
        partial_witness.set_target_arr(&topic_target, &topic);
        ...
        {
            partial_witness.set_hash_target(*ht, h);
        }
    }
}
