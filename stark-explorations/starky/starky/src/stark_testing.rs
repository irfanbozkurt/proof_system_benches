    ...
    [(); S::PUBLIC_INPUTS]:,
{
    ...
    let rate_bits: usize = log2_ceil(constraint_degree + 1);
    ...
    let constraint_evals = (0..size)
        .map(|i| {
            ...
            let accumulators = consumer.accumulators();
            accumulators[0]
        })
        .collect::<Vec<_>>();
    // println!("allah3 constraint_evals.len() {}", constraint_evals.len());

    let poly = PolynomialValues::new(constraint_evals);
    let constraint_eval_degree = poly.degree();
