...
use crate::selectors::{SELECTOR_COLUMN, SELECTOR_SHA};
use crate::{
...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for Sha2CompressionStark<F, D> {
        ...
        P: PackedField<Scalar = FE>,
    {
        yield_constr.constraint_first_row(
            vars.local_values[SELECTOR_COLUMN] - P::from(FE::from_canonical_usize(SELECTOR_SHA)),
        );
...
impl Default for Sha2StarkCompressor {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_multiple() -> Result<()> {
        ...
        let stark: Sha2CompressionStark<plonky2_field::goldilocks_field::GoldilocksField, 2> =
            S::new();
