...
impl<F: Field> Sha2TraceGenerator<F> {
    ...
    fn gen_phase_0(&mut self, his: [u32; 8]) -> ([u32; 16], [u32; 4], [u32; 4]) {
        ...
        let mut wis = [0; 16];
        wis[..8].copy_from_slice(&left_input[..8]);
        wis[8..(8 + 8)].copy_from_slice(&right_input[..8]);
        wis = rotl_wis(wis);
    ...
    pub fn gen_hash(&mut self, left_input: [u32; 8], right_input: [u32; 8]) -> [u32; 8] {
        ...
        self.trace.0.iter_mut().for_each(|row| {
            row[SELECTOR_COLUMN] = F::from_canonical_usize(SELECTOR_SHA);
        });
