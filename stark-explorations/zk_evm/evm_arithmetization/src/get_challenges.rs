...
impl<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize> BenchProof<F, C, D> {
    /// Computes all Fiat-Shamir challenges used in the STARK proof.
    pub(crate) fn get_bench_challenges(
        &self,
        config: &StarkConfig,
    ) -> Result<BenchProofChallenges<F, D>, ProgramError> {
        let mut challenger = Challenger::<F, C::Hasher>::new();
