...
use crate::bench_stark::BENCH_NUM_TABLES;
use crate::util::{get_h160, get_h256, h2u};

/// A STARK proof for each table, plus some metadata used to create recursive
/// wrapper proofs.
#[derive(Debug, Clone)]
pub struct BenchProof<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize> {
    pub multi_proof: MultiProof<F, C, D, BENCH_NUM_TABLES>,
}

pub(crate) struct BenchProofChallenges<F: RichField + Extendable<D>, const D: usize> {
    /// Randomness used in each STARK proof.
    pub stark_challenges: [StarkProofChallenges<F, D>; BENCH_NUM_TABLES],
