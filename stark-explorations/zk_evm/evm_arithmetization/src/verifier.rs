...
use crate::bench_stark::{BenchStark, BenchTable, BENCH_NUM_TABLES};
use crate::cpu::kernel::aggregator::KERNEL;
use crate::cpu::kernel::constants::global_metadata::GlobalMetadata;
use crate::memory::segments::Segment;
use crate::memory::VALUE_LIMBS;
use crate::proof::{AllProof, AllProofChallenges, BenchProof, BenchProofChallenges, PublicValues};
use crate::util::h2u;

pub fn verify_bench_proof<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
>(
    bench_stark: &BenchStark<F, D>,
    bench_proof: BenchProof<F, C, D>,
    config: &StarkConfig,
) -> Result<()>
where
{
    let BenchProofChallenges {
        stark_challenges,
        ctl_challenges,
    } = bench_proof
        .get_bench_challenges(config)
        .map_err(|_| anyhow::Error::msg("Invalid sampling of proof challenges."))?;

    let num_lookup_columns = bench_stark.num_lookups_helper_columns(config);
    ...
    let stark_proofs = &bench_proof.multi_proof.stark_proofs;

    verify_stark_proof_with_challenges(
        arithmetic_stark,
        &stark_proofs[BenchTable::Arithmetic as usize].proof,
        &stark_challenges[BenchTable::Arithmetic as usize],
        Some(&ctl_vars_per_table[BenchTable::Arithmetic as usize]),
        &[],
        config,
    )?;
    verify_stark_proof_with_challenges(
        keccak_stark,
        &stark_proofs[BenchTable::Keccak as usize].proof,
        &stark_challenges[BenchTable::Keccak as usize],
        Some(&ctl_vars_per_table[BenchTable::Keccak as usize]),
        &[],
        config,
    )?;
    verify_stark_proof_with_challenges(
        keccak_sponge_stark,
        &stark_proofs[BenchTable::KeccakSponge as usize].proof,
        &stark_challenges[BenchTable::KeccakSponge as usize],
        Some(&ctl_vars_per_table[BenchTable::KeccakSponge as usize]),
        &[],
        config,
    )?;
    verify_stark_proof_with_challenges(
        logic_stark,
        &stark_proofs[BenchTable::Logic as usize].proof,
        &stark_challenges[BenchTable::Logic as usize],
        Some(&ctl_vars_per_table[BenchTable::Logic as usize]),
        &[],
        config,
    )?;

    verify_cross_table_lookups::<F, D, BENCH_NUM_TABLES>(
        cross_table_lookups,
        bench_proof
            .multi_proof
            .stark_proofs
