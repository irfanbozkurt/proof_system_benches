...
use crate::bench_stark::{generate_bench_traces, BenchStark, BenchTable, BENCH_NUM_TABLES};

use crate::cpu::kernel::aggregator::KERNEL;
use crate::generation::{generate_traces, GenerationInputs};
use crate::get_challenges::observe_public_values;
use crate::proof::{AllProof, BenchProof, PublicValues};

/// Generate traces, then create all STARK proofs.
pub fn prove_bench<F, C, const D: usize>(
    bench_stark: &BenchStark<F, D>,
    config: &StarkConfig,
    timing: &mut TimingTree,
    abort_signal: Option<Arc<AtomicBool>>,
) -> Result<BenchProof<F, C, D>>
where
    ...
    C: GenericConfig<D, F = F>,
{
    ...
    let (traces) = timed!(
        timing,
        "generate all traces",
        generate_bench_traces(bench_stark, config, timing)?
    );

    check_abort_signal(abort_signal.clone())?;

    let proof = prove_bench_with_traces(bench_stark, config, traces, timing, abort_signal)?;
    Ok(proof)
}

/// Compute all STARK proofs.
pub(crate) fn prove_bench_with_traces<F, C, const D: usize>(
    bench_stark: &BenchStark<F, D>,
    config: &StarkConfig,
    trace_poly_values: [Vec<PolynomialValues<F>>; BENCH_NUM_TABLES],
    timing: &mut TimingTree,
    abort_signal: Option<Arc<AtomicBool>>,
) -> Result<BenchProof<F, C, D>>
where
    ...
    C: GenericConfig<D, F = F>,
{
    ...
    let trace_commitments = timed!(
        ...
        trace_poly_values
            ...
            .map(|(trace, table)| {
                ...
                let ret_val = PolynomialBatch::<F, C, D>::from_values(
                    trace.clone(),
                    rate_bits,
                    false,
                    cap_height,
                ...
                println!("{:?}", ret_val.polynomials.len());
    ...
    let (ctl_challenges, ctl_data_per_table) = timed!(
        timing,
        "compute CTL data",
        get_ctl_data::<F, C, D, BENCH_NUM_TABLES>(
            config,
            &trace_poly_values,
            &bench_stark.cross_table_lookups,
            &mut challenger,
            bench_stark.arithmetic_stark.constraint_degree()
        )
    );
...
fn prove_bench_with_commitments<F, C, const D: usize>(
    all_stark: &BenchStark<F, D>,
    config: &StarkConfig,
    trace_poly_values: &[Vec<PolynomialValues<F>>; BENCH_NUM_TABLES],
    trace_commitments: Vec<PolynomialBatch<F, C, D>>,
    ctl_data_per_table: [CtlData<F>; BENCH_NUM_TABLES],
    challenger: &mut Challenger<F, C::Hasher>,
    ctl_challenges: &GrandProductChallengeSet<F>,
    timing: &mut TimingTree,
    abort_signal: Option<Arc<AtomicBool>>,
) -> Result<[StarkProofWithMetadata<F, C, D>; BENCH_NUM_TABLES]>
where
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
{
    let arithmetic_proof = timed!(
        ...
        prove_single_table(
            ...
            config,
            &trace_poly_values[BenchTable::Arithmetic as usize],
            &trace_commitments[BenchTable::Arithmetic as usize],
            &ctl_data_per_table[BenchTable::Arithmetic as usize],
            ctl_challenges,
            challenger,
            timing,
            abort_signal.clone(),
    ...
    let keccak_proof = timed!(
        ...
        prove_single_table(
            ...
            config,
            &trace_poly_values[BenchTable::Keccak as usize],
            &trace_commitments[BenchTable::Keccak as usize],
            &ctl_data_per_table[BenchTable::Keccak as usize],
            ctl_challenges,
            challenger,
            timing,
            abort_signal.clone(),
    ...
    let keccak_sponge_proof = timed!(
        ...
        prove_single_table(
            ...
            config,
            &trace_poly_values[BenchTable::KeccakSponge as usize],
            &trace_commitments[BenchTable::KeccakSponge as usize],
            &ctl_data_per_table[BenchTable::KeccakSponge as usize],
            ctl_challenges,
            challenger,
            timing,
            abort_signal.clone(),
    ...
    let logic_proof = timed!(
        ...
        prove_single_table(
            ...
            config,
            &trace_poly_values[BenchTable::Logic as usize],
            &trace_commitments[BenchTable::Logic as usize],
            &ctl_data_per_table[BenchTable::Logic as usize],
            ctl_challenges,
            challenger,
            timing,
            abort_signal.clone(),
